...
public class DBEditDialog extends JDialog{
	...
	public DBEditDialog(Entity entity,ArrayList<String> dataString, InfoTablePanel table) {
		oldData = dataString;
		...
		for (Attribute attribute : entity.getAttributes()) {
			...
			if (attribute.isPK())
				...
				textField.setText(dataString.get(i));
		...
		listPane.add(Box.createRigidArea(new Dimension(0, 5)));
		JButton search = new JButton("Izmeni");
		search.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!empty){
				if(!((Table)entity).edit(oldData,queryData))
					return;
